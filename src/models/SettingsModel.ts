...
export class SettingsModel {
  ...
  static async createOrUpdate(settings: Omit<Settings, 'id'>): Promise<void> {
    ...
    if (existing && existing.id) {
      await db.runAsync(
        'UPDATE settings SET default_currency_code = ?, default_tax_id = ? WHERE id = ?',
        [settings.default_currency_code || null, settings.default_tax_id || null, existing.id]
      );
    } else {
      await db.runAsync(
        'INSERT INTO settings (default_currency_code, default_tax_id) VALUES (?, ?)',
        [settings.default_currency_code || null, settings.default_tax_id || null]
      );
    }
  }
  ...
  static async getDefaultTaxId(): Promise<number | null> {
    const settings = await this.get();
    return settings?.default_tax_id || null;
  }
}
